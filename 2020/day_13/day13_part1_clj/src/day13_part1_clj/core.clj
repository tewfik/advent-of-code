...
(ns day13-part1-clj.core
  (:require [clojure.string :as str])
  (:gen-class))

(defn parse-input [input-str]
  (let [[earliest-str,bus-str] (str/split-lines input-str)
        earliest (Integer/parseInt earliest-str)
        buses (->>
             bus-str
             (#(str/split % #","))
             (filter #(not= % "x"))
...
(defn calc-wait-time [earliest bus]
  (let [miss-time (mod earliest bus)
        wait-time (- bus miss-time)]
    [bus wait-time]))

(defn solve-part-1 [earliest buses]
  (->>
   buses
   (map #(calc-wait-time earliest %))
   (apply min-key second)
   (reduce *))

(defn -main
  [& args]
  (println "test: " (let [[earliest buses] [939 '(7, 13, 59, 31, 19)]]
     (solve-part-1 earliest buses)))
  (println "part 1: " (let [[earliest buses] (parse-input (slurp "resources/input.txt"))]
     (solve-part-1 earliest buses))))
