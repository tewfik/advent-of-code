...
(ns day13-part2-clj.core
  (:require [clojure.string :as str])
  (:gen-class))

(defn parse-input [input-str]
  (let [bus-str (second (str/split-lines input-str))
        buses-raw (str/split bus-str #",")]
    (map #(if (= % "x")
            1
            (Integer/parseInt %))
         buses-raw)))
...
;;
;; (defn check-one [[bus rank] t]
;;   ;(prn bus " " rank " " (+ t rank))
;;   (= 0 (mod (+ t rank) bus)))

;; (defn check-contest [buses t]
;;   ;(prn buses)
;;   (every? #(check-one % t) (map vector buses (range))))

;; (defn solve-part-2 [buses]
;;   (loop [t 1]
;;     (if (check-contest buses t)
;;       t
;;       (recur (inc t)))))
...
(defn -main
  [& args]
  (def buses (parse-input (slurp "resources/input.txt")))
  (println "part 2:" (solve-part-2 buses)))
