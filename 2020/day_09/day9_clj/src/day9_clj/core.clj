...
(defn check-number [input preamble index]
  (let [current-num (get input index)
        window (subvec input (- index preamble) index)
        previous-num-combo (combo/combinations window 2)]
    (when
        (not-any? #(= current-num (reduce + %)) previous-num-combo)
      current-num)))

(defn find-weakness [input preamble]
  (->>
   (range preamble (count input))
   (map #(check-number input preamble %))
   (filter identity)
   (first)))

(defn find-sum [input weakness offset limit]
  (let [window (take limit (drop offset input))
        current-sum (reduce + window)]
    (cond
      (= weakness current-sum) (+ (apply min window) (apply max window))
      (> weakness current-sum) (recur input weakness offset (inc limit))
      (< weakness current-sum) (recur input weakness (inc offset) 1))))

(defn solve-part-1 [input preamble]
  (find-weakness input preamble))

(defn solve-part-2 [input preamble]
  (find-sum
   input
   (find-weakness input preamble)
   0
...
(defn -main
  ...
  (println "part 1:" (solve-part-1 input 25))
  (println "part 2:" (solve-part-2 input 25)))
