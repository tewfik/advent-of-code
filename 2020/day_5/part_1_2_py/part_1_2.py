...
def get_seat(bpass):
    return int(bpass.translate("".maketrans("FBLR", "0101")), 2)


def main():
    inp = [x.strip() for x in open("input.txt").readlines()]
    taken_seat = set(get_seat(x) for x in inp)
    print("part 1:", max(taken_seat))

    taken_seat_s = set(taken_seat)
    available_seat = set(range(min(taken_seat), max(taken_seat) + 1))
    print("part 2:", available_seat - taken_seat_s)
