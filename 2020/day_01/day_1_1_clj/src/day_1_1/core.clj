...
(defn solution [expenses n]
  (->>
   (combo/combinations expenses n)
   (map #(list (apply * %) (apply + %)))
   (filter #(= 2020 (second %)))
   (first)                              ; we only need the first solution
   (first)))                            ; keep the product

(defn -main
  [& args]
  (def expenses
    (parse-input (slurp "resources/input.txt")))
  (println (solution expenses 2))
  (println (solution expenses 3)))
