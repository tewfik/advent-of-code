...
fn find_expenses(expenses: impl Iterator<Item = u32>, nb_entries: usize) -> Option<Vec<u32>>{
    expenses
        .combinations(nb_entries)
        .find(|x| x.iter().sum::<u32>() == 2020)
}

fn solve(expenses: impl Iterator<Item = u32>, nb_entries: usize) -> Option<u32> {
    find_expenses(expenses, nb_entries)
        .map(|expenses| expenses.iter().product())
}

fn main() {
    dbg!(solve(parse_input(), 2));
    dbg!(solve(parse_input(), 3));
}
